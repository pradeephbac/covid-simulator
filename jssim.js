...
(function (jss) {
  ...
  Scheduler.prototype.update_mini = function () {
    ...
    for (var i = 0; i < events.length; ++i) {
      if (events[i].repeatInterval) {
        this.scheduleRepeatingAt(
          events[i],
          this.current_time + events[i].repeatInterval,
          events[i].repeatInterval
        );
      }
    }
  ...
  Scheduler.prototype.scheduleRepeatingAt = function (
    evt,
    startTime,
    deltaTime
  ) {
      ...
      for (var v = 0; v < this.network.V; ++v) {
        ...
        for (var i = 0; i < adj_v.length; ++i) {
          ...
          if (e.either() == v) {
            ...
            context.moveTo(
              x1 * this.cellWidth,
              canvas.height - y1 * this.cellHeight
            );
            context.lineTo(
              x2 * this.cellWidth,
              canvas.height - y2 * this.cellHeight
            );
            context.stroke();
          }
        }
      ...
      for (var j = 0; j < this.height; ++j) {
        ...
        if (this.showPotentialField && this.potentialField[i][j] != 0) {
          ...
          var g =
            100 +
            Math.floor(
              (155 * (potential - this.minPotential)) /
                (this.maxPotential - this.minPotential)
            );
          var b = Math.floor(
            (255 * (potential - this.minPotential)) /
              (this.maxPotential - this.minPotential)
          );
  ...
  Messenger.prototype.update = function (deltaTime) {
    for (recipient in this.inbox) {
      ...
      if (recipient_inbox.size() > 10) {
        ...
        while (
          sender_msg != null &&
          sender_msg.time < this.scheduler.current_time
        ) {
